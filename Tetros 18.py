...
__credits__ = ["Jason Schattman", "Huzaifa Arshad", "Gaurav Iyer", "Leon Fattakhov", "Zach Chapman"]
__license__ = "GPL"
__version__ = "18"
...
instructions = Canvas(root, width=800, height=600, background="white") #Make instructions canvas
eText = Entry(root, font = "Times 20 bold", fg = "green") #Make text box for user to enter speed at which tetrominoes should fall
okayB = Button(root,text="Begin!", font = "Times 12 bold", command=lambda:getDifficulty()) #Make button for user to click in order to advance to the game screen 
screen = Canvas(root, width=600, height=525, background="white") #Make main game canvas
quitB = Button(root, text="Quit Tetros", font = "Times 12 bold", command = lambda:endAll()) #Make button for quitting Tetros (present in the final game statistics screen)
...
def exitB():
    ...
    try:
        ...
        endAll()
def save():
    Test= filedialog.asksaveasfilename(defaultextension= ".txt", filetypes = [("TetrosSaveFile",".txt")], title="Save game")
    print(Test)
...
def setInitialValues():
    ...
    functions  = [makei, makej, makel, makeo, makes, maket, makez] #List of functions to make tetrominoes
    predictShape = 0 #Initializing a variable to assign to the next shape tetromino (top right of the interface)
...
def overlay():
    ...
    gridOverlay = False #Boolean that controls whether grid overlay should be present (used for developing)
...
def rotatePoint(point, centre, thetaDegrees):
    ...
    thetac = cmath.exp(thetaRadians * 1j) #Express angle as a complex number (for calculations)
    centreX = centre[0] #Pull out x coordinate of centre
    centreY = centre[1] #Pull out y coordinate of centre
    
    centrec = complex(centreX, centreY) #Create a complex expression to represent the centre
    v = thetac * (complex(x, y) - centrec) + centrec #v consists of the x and y values of the rotated coordinate in its real and imaginary parts respectively
...
def makeWholeCoords():
    ...
    screen.delete(ALL) #Delete all objects on the screen (to make sure all objects get updated)
    #Go through blockCoords and redraw all the blocks that it contains the coordinates for 
    for i in range(0, len(blockCoords)-1): 
        for g in range(0, len(blockCoords[i])):
            coords = []
            for p in range(0, 4):
                coords.append(blockCoords[i][g][p])
            blocks[i][g] = screen.create_polygon(coords, fill = colours[i], outline = "black", width = "2")#random.choice(["red", "green", "black"]), outline = "black", width = "2")
...
def makePolygon(coords, colour):
    ...
    block1 = screen.create_polygon(coords[0], fill = colour, outline = "black", width = "2")
    block2 = screen.create_polygon(coords[1], fill = colour, outline = "black", width = "2")
    block3 = screen.create_polygon(coords[2], fill = colour, outline = "black", width = "2")
    block4 = screen.create_polygon(coords[3], fill = colour, outline = "black", width = "2")
...
def makei(x, y, real = True):
    ...
    if real: #If real is true information is added to the arrays, otherwise it is to be made for the "Next Shape" box
        ...
        coords.append(makeCoords(x+length, y-length))
        blockCoords.append(coords) #Append the coordinates to the main blockCoords list
        blocks.append(makePolygon(coords, "cyan")) #Create the polygon on the screen
        centres.append([x, y]) #Append the centre of the polygon to the centres list
        colours.append("cyan") #Append the colour of the polygon to the colours list
    else:
        ...
        c.append(makeCoords(x+length, y-length))
        predictShape = makePolygon(c, "cyan") #Create polygon on the screen and assign it to predictShape
def makej(x, y, real = True):
    ...
    if real: #If real is true information is added to the arrays, otherwise it is to be made for the "Next Shape" box
        ...
        coords.append(makeCoords(x+length/2, y-length/2))
        blockCoords.append(coords)#Append the coordinates to the main blockCoords list
        blocks.append(makePolygon(coords, "blue")) #Create the polygon on the screen
        centres.append([x, y]) #Append the centre of the polygon to the centres list
        colours.append("blue") #Append the colour of the polygon to the colours list
    else:
        c = [] #Initialize coordinates list
        #Append coordinates to the list according to the shape being created
        ...
        c.append(makeCoords(x+length/2, y-length/2))
        predictShape = makePolygon(c, "blue") #Create polygon on the screen and assign it to predictShape
def makel(x, y, real = True):
    ...
    if real: #If real is true information is added to the arrays, otherwise it is to be made for the "Next Shape" box
        ...
        blockCoords.append(coords)#Append the coordinates to the main blockCoords list
        blocks.append(makePolygon(coords, "orange")) #Create the polygon on the screen
        centres.append([x, y]) #Append the centre of the polygon to the centres list
        colours.append("orange") #Append the colour of the polygon to the colours list
    else:
        c = [] #Initialize coordinates list
        #Append coordinates to the list according to the shape being created
        c.append(makeCoords(x-length*3/2, y-length/2))
        ...
        predictShape = makePolygon(c, "orange") #Create polygon on the screen and assign it to predictShape
def makeo(x, y, real = True):
    ...
    if real: #If real is true information is added to the arrays, otherwise it is to be made for the "Next Shape" box
        ...
        blockCoords.append(coords)#Append the coordinates to the main blockCoords list
        blocks.append(makePolygon(coords, "yellow")) #Create the polygon on the screen
        centres.append([x, y]) #Append the centre of the polygon to the centres list
        colours.append("yellow") #Append the colour of the polygon to the colours list
    else:
        c = [] #Initialize coordinates list
        #Append coordinates to the list according to the shape being created
        c.append(makeCoords(x-length, y-length))
        ...
        predictShape = makePolygon(c, "yellow") #Create polygon on the screen and assign it to predictShape
def makes(x, y, real = True):
    ...
    if real: #If real is true information is added to the arrays, otherwise it is to be made for the "Next Shape" box
        ...
        coords.append(makeCoords(x+length/2, y-length*3/2))
        blockCoords.append(coords)#Append the coordinates to the main blockCoords list
        blocks.append(makePolygon(coords, "green")) #Create the polygon on the screen
        centres.append([x, y]) #Append the centre of the polygon to the centres list
        colours.append("green") #Append the colour of the polygon to the colours list
    else:
        c = [] #Initialize coordinates list
        #Append coordinates to the list according to the shape being created
        c.append(makeCoords(x-length*3/2, y-length/2))
        ...
        c.append(makeCoords(x-length/2, y-length*3/2))
        c.append(makeCoords(x+length/2, y-length*3/2))
        predictShape = makePolygon(c, "green") #Create polygon on the screen and assign it to predictShape
...
def maket(x, y, real = True):
    ...
    if real: #If real is true information is added to the arrays, otherwise it is to be made for the "Next Shape" box
        ...
        coords.append(makeCoords(x+length/2, y-length/2))
        blockCoords.append(coords)#Append the coordinates to the main blockCoords list
        blocks.append(makePolygon(coords, "magenta")) #Create the polygon on the screen
        centres.append([x, y]) #Append the centre of the polygon to the centres list
        colours.append("magenta") #Append the colour of the polygon to the colours list
    else:
        c = [] #Initialize coordinates list
        c.append(makeCoords(x-length*3/2, y-length/2))
        c.append(makeCoords(x-length/2, y-length/2))
        c.append(makeCoords(x-length/2, y-length*3/2))
        c.append(makeCoords(x+length/2, y-length/2))
        predictShape = makePolygon(c, "magenta") #Create polygon on the screen and assign it to predictShape
def makez(x, y, real = True):
    ...
    if real: #If real is true information is added to the arrays, otherwise it is to be made for the "Next Shape" box
        ...
        coords.append(makeCoords(x+length/2, y-length/2))
        blockCoords.append(coords)#Append the coordinates to the main blockCoords list
        blocks.append(makePolygon(coords, "red")) #Create the polygon on the screen
        centres.append([x, y]) #Append the centre of the polygon to the centres list
        colours.append("red") #Append the colour of the polygon to the colours list
    else:
        c = [] #Initialize coordinates list
        c.append(makeCoords(x-length*3/2, y-length*3/2))
        c.append(makeCoords(x-length/2, y-length*3/2))
        c.append(makeCoords(x-length/2, y-length/2))
        c.append(makeCoords(x+length/2, y-length/2))
        predictShape = makePolygon(c, "red") #Create polygon on the screen and assign it to predictShape
def makeScore():
    ...
    if not qPressed: #If the main game is still running
        ...
        scoreP = screen.create_text(450, 200, text = "Score: " + str(score), font="Times 20 bold") #Draw new score on the screen
    
def checkFloor():
    """Checks if tetromino is about to hit the floor, returns True if it is and False otherwise."""
    global blockCoords, floor
    #Go through all the y coordinates in the current falling tetromino and check if any of them are the same as the floor
    for i in range(0, 4):
        for e in range(0, 4):
...
def checkWalls():
    ...
    for i in range(0,4):
        for u in range(0,4):
            if blockCoords[-1][i][u][0] < 50: #Check if any x coordinate is the same as the left wall
                return "left" #Return whic wall it is about to hit
            
            if blockCoords[-1][i][u][0] > 250: #Check if any x coordinate is the same as the right wall
...
def crash(coord = 1, value = 25):
    ...
    if len(blockCoords) > 1: #If this is not the first tetromino
        ...
        #In other words, check if any block overlaps a block that has already been placed 
...
def checkRow():
    ...
    for i in range(0, len(blockCoords)-1):
        for u in range(0, len(blockCoords[i])):
            ys.append(min([blockCoords[i][u][0][1], blockCoords[i][u][1][1], blockCoords[i][u][2][1], blockCoords[i][u][3][1]]))
    yds = dict((i, ys.count(i)) for i in ys) #Make a dictionary of the frequency of y values
        ...
        if yds[e]  == 10:
            for p in range(0, len(blockCoords)-1):
                for k in range(0, len(blockCoords[p])):
                    #Check if block is part of row being deleted using its top y coordinate
                    if e == min([blockCoords[p][k][0][1], blockCoords[p][k][1][1], blockCoords[p][k][2][1], blockCoords[p][k][3][1]]):
                        centreY = e + 12.5 #Calculate the block's centre y coordinate
                        topX = min([blockCoords[p][k][0][0], blockCoords[p][k][1][0], blockCoords[p][k][2][0], blockCoords[p][k][3][0]]) #Find its left x coordinate 
                        centreX = topX + 12.5 #Calculate the block's centre x coordinate
                        explodeCentres.append([centreX, centreY]) #Add centre coordinates to explodeCentres array
                        toDel.append([p, k]) #Add its information to toDel
                    #Check if the block is above the row being deleted
                    if e > min([blockCoords[p][k][0][1], blockCoords[p][k][1][1], blockCoords[p][k][2][1], blockCoords[p][k][3][1]]):
                        for b in range(0, 4):
                            yds[blockCoords[p][k][b][1]] -= 1 #Update dictionary value for that y coordinate key (decrease its frequency count by 1)
                            blockCoords[p][k][b][1] += 25 #Make block fall down by 25 pixels
                            try:
                                yds[blockCoords[p][k][b][1]] += 1 #Update dictionary value for that y coordinate key (increase its frequency count by 1)
                            except KeyError:
                                pass #If key is not in dictionary then pass
                            
            blockCoords2 = copy.deepcopy(blockCoords) #Create a deep copy of blockCoords (this one will not be edited to avoid indexing errors)
        
            toDel2 = sorted(toDel, key = getKey) #Sort toDel by the second value in each nested list
            toDel3 = toDel2[::-1] #Reverse toDel2 (delete the highest indices first to avoid indexing errors)
            ...
            particles = [] #Array to store particle objects (being displayed on screen)
            angles = [] #The angle that they move in (relative to the point they explode from)
            ...
            for blockNum in range(0, 10):
                #Use temporary arrays so the entire list can be appended to the main array later (better for indexing)
                rTemp = []
                xSizesTemp = []
                ySizesTemp = []
                rSpeedsTemp = []
            ...
            for w in range(0, 5): #Show explosion for 5 frames
                for i in range(0, 10): #Loop through the 10 explosions, one for each block being deleted
                    for q in range(0, 25): #Loop through the particles in each explosion (25)
                        #Use trigonometry to calculate te x and y position of the specific particle
                        xP[i][q] = explodeCentres[i][0] + r[i][q] * math.cos(angles[i][q])
                        yP[i][q] = explodeCentres[i][1] - r[i][q] * math.sin(angles[i][q])
                        #Increase the particles radius by whatever speed it is going at
                        r[i][q] = r[i][q] + rSpeeds[i][q]
                        #Draw the particle on the screen
                        particles[i][q] = screen.create_oval(xP[i][q] - xSizes[i][q], yP[i][q] - ySizes[i][q], xP[i][q] + xSizes[i][q], yP[i][q] + ySizes[i][q], fill = hexadecimal())
                ...
                for i in range(0, 10): #Loop through the 10 explosions, one for each block being deleted
                    for q in range(0, 25): #Loop through the particles in each explosion (25)
                        screen.delete(particles[i][q]) #Delete the particle (for animation effect)
            
            #Remove the specific block coordinates from blockCoords using toDel3 and blockCoords2
            for r in range(0, len(toDel3)):
                blockCoords[toDel3[r][0]].remove(blockCoords2[toDel3[r][0]][toDel3[r][1]])
...
def endGame():
    ...
    screen.create_rectangle(-10, 225, 610, 275, fill="white", outline="white")  # Create white rectangle behind text to make sure text is legible
    screen.create_text(300, 250, text = "Your final score is: " + str(score) + ". Thank you for playing Tetros!", font = "Times 18 bold")
    ...
    screen.create_rectangle(-10, 375, 610, 425, fill = "white", outline = "white")  # Create white rectangle behind text to make sure text is legible
    screen.create_text(300, 400, text = "You cleared " + str(clearedRows) + rowForm + comment, font = "Times 18 bold")
...
def equalIgnoreOrder(a, b):
    ...
    if len(a) != len(b):  # If the lengths of the lists are not equal then their elements are not equal for sure
        return False
    unmatched = list(b)  # Create a list of b (so the original b stays intact)
    
    for element in a:  # Loop through list a
        try:
            unmatched.remove(element)  # Try to remove the current element in list a from unmatched
        except ValueError:  # If the element cannot be removed then return False
            return False
    return not unmatched  # Else return the opposite of unmatched (returns True when list is empty which it will be)

def makeTetrisRectangle():
    """Makes the Tetros box in which tetrominoes are placed."""
    screen.create_rectangle(25, 0, 275, 500, fill=None, outline="black", width="3")

def coverNext():
    """Covers the previous 'next' shape."""
    global length
    screen.create_polygon(450-length*2, 150-length, 450+length*2, 150-length, 450+length*2, 150+length*3/2, 450-length*2, 150+length*3/2, fill="white", outline="white", width=2)
...
def showNext():
    ...
    if not qPressed:  # If the main game is still running
        ...
        screen.create_text(450, 100, text="Next Shape:", font="Times 20 bold") # Create "next shape" text
        ...
        for p in range(0, len(functions)):
            ...
            if call == functions[p].__name__:
                # An I-shape tetromino or O-shape tetromino require different x and y coordinates when compared to the others (they have different centres)
                if shapeN == "i" or shapeN == "o":
                    functions[p](450, 150, real = False)
                else:
                    functions[p](462.5, 162.5, real = False)
                break  # Break the for loop for efficiency (don't need to loop for no reason)
...
def getRandomShape():
    ...
    # This is needed because blockCoords may have been updated and a crash may have been detected but the graphical tetromino may not have been updated
    if len(blocks) > 1:
        for j in range(0, 4):
    ...
    if (counter + 1) % 7 == 0: # Just before reaching the last element of the pShapes list (every time it reaches the sixth element)
        tempShapes = ["i", "j", "l", "o", "s", "t", "z"] # Create a temporary array of possible shapes
        nextShape = pShapes[-1] # Assign nextShape to the last element in the current pShapes array
        tempShapes.remove(nextShape) # Remove the last element from the tempShapes array
        tempStart = [nextShape] # Create a tempStart array that just has one element which is nextShape
        random.shuffle(tempShapes) # Randomize the tempShapes array
        tempStart.extend(tempShapes) # Add the tempShapes array to the end of the tempStart array
        pShapes = tempStart # Assign pShapes to the tempStart array
        counter = 7 # Set counter to 7
        
    shape = pShapes[counter%7] # Choose a shape from pShapes (using modulus to make sure that there are no index errors)
    call = "make" + shape # Call is a string that is a function name
    for p in range(0, len(functions)): # Go through the list of functions
        if call == functions[p].__name__:  # If call matches with the current function's name
            # Depending on the shape, call the function with x and y coordinates (I and O tetrominoes has different centres compared to the rest)
...
def animateShape():
    ...
    if crash():  # If the current shape has crashed then it gets the next tetromino to fall
        ...
        for i in range(0, 4):
            for u in range(0, 4):
                blockCoords[-1][i][u][1]  += 25 #Make tetromino fall by 25 pixels
                
        centres[-1][1] += 25 #Increase the y coordinate of the centre of tetromino by 25 pixels
        
        for j in range(0, 4):
            screen.delete(blocks[-1][j]) #Delete previous frame 
        blocks[-1] = makePolygon(blockCoords[-1], colours[-1]) #Create updated tetromino on the screen


def makeInstructions():
    """Draws images and text on the instructions screen."""
    global instructions, tetros, background
    instructions.create_image(400, 300, image = background) #Create background image
    instructions.create_image(400, 100, image = tetros) #Create Tetros logo
    instructions.create_rectangle(40, 200, 750, 560, fill = "white") #Create white rectangle background for text

    #Create instructions for the user
    instructions.create_text(400, 230, text = "WELCOME TO TETROS!", font = "Times 20 bold underline", fill = "#008855")
    instructions.create_text(400, 275, text = "INSTRUCTIONS", font = "Times 18 bold")
    instructions.create_text(400, 300, text = "Objective: Try to fit as many tetrominoes as possible on the screen by clearing rows!", font = "Times 11 bold")
    instructions.create_text(400, 325, text = "Press the UP key or X to rotate the tetromino clockwise.", font = "Times 11 bold")
    instructions.create_text(400, 350, text = "Press Z to rotate the tetromino anti-clockwise.", font = "Times 11 bold")
    instructions.create_text(400, 375, text = "Press the DOWN key to accelerate the current tetromino.", font = "Times 11 bold")
    instructions.create_text(400, 400, text = "Press the LEFT or RIGHT keys to move the tetromino in their respective directions.", font = "Times 11 bold")
    instructions.create_text(400, 425, text = "Press Q to quit the game.", font = "Times 11 bold")
    instructions.create_text(400, 450, text = "Press P to pause/unpause the game.", font = "Times 11 bold")
    instructions.create_text(400, 475, text = "Scoring: 8 points for a new tetromino, 20 points for a cleared row, and 1 point for accelerating the tetromino.", font = "Times 11 bold")
    instructions.create_text(400, 500, text = "Tip: Accelerate a tetromino if you are confident it is in the right column for free points!", font = "Times 10 bold")

    #Instruct user to enter speed that they want tetrominoes to fall at
    instructions.create_text(400, 525, text = "Enter speed at which tetrominoes should fall (in seconds) - must be non-negative:", font = "Times 14 bold underline")
    instructions.create_text(400, 550, text = "Approximate difficulty for different speeds (in seconds): Easy - 0.5 Medium - 0.3 Hard - 0.1", font = "Times 12 bold")
...
def getDifficulty():
    ...
    try:
        ...
        if s >= 0: #If it is  non-negative
            #Destroy the instructions screen, the text box and the "Next" button
            eText.destroy()
            okayB.destroy()
...
def sidebar():
    ...
    screen.create_text(440, 275, text = "CONTROLS", font = "Times 13 bold underline") #Create "CONTROLS" headline
...
def keyDownHandler(event):
    ...
    #Make sure game is not paused, tetromino is not about to crash and that the key pressed is to rotate the tetromino
    if not paused and not crash() and (event.keysym == "Up" or event.keysym == "x" or event.keysym == "X" or event.keysym == "z" or event.keysym == "Z"): 
        #Initialize hit
        hit = False
        
        for i in range(0, 4):
            #Depending on the key pressed, rotate tetromino clockwise or anti-clockwise
            if event.keysym == "z" or event.keysym == "Z":
                blockCoords[-1][i] = rotatePolygon(blockCoords[-1][i], centres[-1], -90)
            else:
                blockCoords[-1][i] = rotatePolygon(blockCoords[-1][i], centres[-1], 90)

        #Make sure that the rotated tetromino is not going through the floor or any wall
        if not checkWalls() == None or checkFloor():
            hit = True

        #If this is not the first tetromino and hit is not already true then check to see if rotating it will cause it to overlap already placed blocks
        if len(blockCoords) > 1 and not hit:
            for i in range(0, len(blockCoords)-1):
                for u in range(0, len(blockCoords[i])):
                    for g in range(0, 4):
                        #Check if the blocks are the same (order of points does not matter in this case)
                        if equalIgnoreOrder(blockCoords[i][u], blockCoords[-1][g]):
                            hit = True
        ...
        if not hit: #If the rotating the tetromino will not cause errors
            ...
            #Append the new tetromino to the blocks array (also creates new tetromino on the screen)
    ...
    #If game is not paused, the left arrow key has been pressed, the tetromino will not crash if it is moved left, the tetromino is not about to crash and the tetromino is not hitting the left wall
    ...
    #If game is not paused, the right arrow key has been pressed, the tetromino will not crash if it is moved right, the tetromino is not about to crash and the tetromino is not hitting the right wall
    ...
    #If the game is not paused, the down arrow key has been pressed and the tetromino will not crash if it is moved down
...
def initialScreen():
    ...
    eText_window = instructions.create_window(400, 580, window = eText) #Put the text box in the instructions screen
    eText.focus_set() #Focus automatically to the text box
    okayB.config(bd=5)
    okayB_window = instructions.create_window(580, 580, window = okayB) #Put the "Begin!" button on the instructions screen
...
def interfaceButtons():
    ...
    buttonQuit_window = screen.create_window(400, 240, window=buttonQuit)

    #Create pause button and assign it to the changePause function
    buttonPause = Button(root, text = "Pause Game", command = lambda:changePause())
    buttonPause.config(bd=5)
    #Create the pause button on the screen
    buttonPause_window = screen.create_window(500, 240, window=buttonPause)
...
def runGame():
    ...
    winsound.PlaySound(tetrisSong, winsound.SND_FILENAME| winsound.SND_ASYNC| winsound.SND_LOOP) #Loop the background music
    while not qPressed and not paused: #Run while not paused and not quitted
        animateShape() #Animate shape falling down
        makeScore() #Update score
        screen.update() #Update screen
...
#Bind the return key press in the instructions screen to callDifficulty (allows user to enter input and press enter to progress to the game screen)
eText.bind("<Return>", callDifficulty)
#Bind any key pressed in the game screen to keyDownHandler which decides what action is to be taken, if any
